    ...
    private void spawnTiles()
    {
        ...
        for (int i = 0; i < stagesNumber; i++)
        {
            for (int j = 0; j < Random.Range(2, 5); j++)
            {
                ...
                if (eventR == 1)
                {
                    GameObject _event = Instantiate(eventPrefab, new Vector3(-tileBoundaries.x / 2 + 1, tileBoundaries.y / 2 - 1, 0), Quaternion.identity);
                    _event.transform.SetParent(tile.transform, false);
                }
                else
                {
                    GameObject fight = Instantiate(fightPrefab, new Vector3((float)((float)-tileBoundaries.x / 2 + 1), tileBoundaries.y / 2 - 1, 0), Quaternion.identity);
